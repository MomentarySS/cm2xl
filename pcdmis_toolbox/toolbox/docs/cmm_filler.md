...
- **圆柱等多轴特性（如 CC_15 的 X/Y/D）**：每个序号只取 **一个** 实测值写入模板；模板「轴」列可指定 `D`/`X`/`Y`，**未配置时默认取 D（直径）**。角度项优先取 **A**。OCR 漏识标签时按相邻 AX 表头分块补全序号，多缺口不会共用第一块数据。若同一序号要分行填 X/Y/D，见仓库 `docs/PDF_ENHANCEMENT_PLAN.md` **Phase 6**（待开发）。
