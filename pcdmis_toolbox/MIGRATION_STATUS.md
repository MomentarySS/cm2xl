...
## 五、2026-08-27 晚（v1.0.2）Bug 修复

### pc_to_excel

| 问题 | 根因 | 修法 |
|------|------|------|
| **表头关键字误杀真实序号** | `inspection_form_fill.py` 在数据行检测 "检验/判定/检具/OK/NG"，"OK-001" 被误跳过 | 关键字检测移至 `header_scan_row`（第5行），不检查数据行 |
| **_compute_outtol 语义错** | 有偏差无公差时返回 `0.0`，用户误以为"偏差为0" | 改为返回 `None`（表示无测量数据） |
| **_on_error 死代码** | `msg.startswith("【")` 永远为 True，else 分支永不执行 | 移除无效分支，所有调用方已预格式化 |
