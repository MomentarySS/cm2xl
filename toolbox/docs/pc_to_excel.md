...
「部署工具栏启动器」与「一键导出 Excel」在同一行。测完后点工具栏按钮：启动 cm2xl（跳过 OCR）→ 打开「PCDMIS导出」→ 按当前「仅 Mark」设置自动导出 Excel。cm2xl 已在运行时会唤醒现有窗口再导，不另开实例。重装后工具栏不必重拖（启动器路径不变）。

连续导出请等「导出完成」弹窗后再点下一次；抽数期间状态灯可能暂不刷新程序名，属正常。
...
- **连接失败 0x80070005**：权限不一致 → 使用安装目录中的 `run_as_admin.bat`，或改为与 PCDMIS 相同权限启动。
- **32 位 Python**：PC-DMIS 2017 R2+ 需 64 位，启动时会硬阻断。
- **导出为空**：确认 PRG 中有已执行的尺寸/形位命令，且「仅导出标记项」选项与程序一致。
- **第二次导出卡住**：请用 1.0.12+。旧版会因 COM 与界面互锁假死；可结束 `cm2xl.exe` 后升级，不必关 PC-DMIS。
