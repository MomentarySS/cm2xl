...
def _get_appdata_dir() -> str:
    r"""优先使用 %LOCALAPPDATA%\CMMFiller，回退到项目目录"""
...
TEMPLATE_PATH = str(_resource_path('templates', '模板2.xlsx'))
if getattr(sys, 'frozen', False):
    PDF_FOLDER = ''
...
示例:
  ...
  # 指定 PDF 文件夹和输出文件夹
  python cmm_filler_v10.py -f D:/PDFs -o D:/output

  # 指定模板和 PDF 文件夹
  python cmm_filler_v10.py -t 自定义模板.xlsx -f D:/PDFs

  # 启动 GUI 界面
  python cmm_filler_v10.py --gui

  # 启动模板配置向导
  python cmm_filler_v10.py --wizard

  # 导出标准模板（规格+公差，不含实测值）
  python cmm_filler_v10.py --export-template -f D:/PDFs
""",
    )
    parser.add_argument('-t', '--template', default=None, help='Excel 模板路径（默认: 项目目录下的模板2.xlsx）')
    parser.add_argument('-f', '--folder', default=None, help='PDF 文件夹路径（默认: 项目目录）')
    parser.add_argument('-o', '--output', default=None, help='输出文件夹路径（默认: 项目目录）')
    parser.add_argument('--dpi', type=int, default=DPI, help=f'PDF 渲染分辨率（默认: {DPI}）')
    parser.add_argument('--gui', action='store_true', help='启动图形界面')
    parser.add_argument('--wizard', action='store_true', help='启动模板配置向导')
    parser.add_argument('--export-template', action='store_true', help='从 PDF 导出标准模板文件（规格+公差）')
    parser.add_argument('--version', action='version', version=f'CMMFiller {__version__}')

    # 打包版双击 exe 默认打开 GUI（命令行传参时仍走 CLI）
    ...
    try:
        if args.gui:
            ...
            template = args.template or TEMPLATE_PATH
            pdf_folder = args.folder or PDF_FOLDER
            ...
            logger.info('=== 导出标准模板 ===')
            filler = CMMReportFiller(template, dpi=args.dpi)
            filler.export_standard_template(pdf_folder, output_folder)
            ...
            logger.info('=== CMMFiller 批量处理 ===')
            filler = CMMReportFiller(template, dpi=args.dpi)
            os.makedirs(output_folder, exist_ok=True)
            filler.batch_process_by_date(pdf_folder, output_folder)
            logger.info(f'日志已保存至: {LOG_PATH}')
    except Exception:
        import traceback
        try:
