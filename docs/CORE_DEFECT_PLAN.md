...
> ```
>
> 当前全量 **314 passed**（`tests/` 42 + `modules/*/tests` 272；本次新增 36 = P1-5 的 25 + P1-6 的 6 + P1-7 提示补全的 1 + P1-7 降级路径的 2 + P1-8 的 2）。
...
**已完成 11 / 20，未开始 9 / 20。** 基线 233 → **314 passed**（`tests/` 42 + `modules/*/tests` 272，新增 36 测试 = P1-5 的 25 + P1-6 的 6 + P1-7 提示补全的 1 + P1-7 降级路径的 2 + P1-8 的 2）。

**2026-09-22 晚 → 2026-09-23 下午：两轮真机验证 + 4 个新 bug（已修 2 + 登记 2）。** 进展如下：

| 时点 | 验证 | 真机结论 |
|------|-------|---------|
| 09-22 23:30 | **P1-2** 第一次启动 | 配置在 23:30:01 被迁移写入（`_version` `1.0.0` → `2.0.0`，6 个业务键全在） |
| 09-22 23:33 | **P1-2** 第二次启动 | **0 条迁移弹窗审计**、配置未被再次改写、退出保存未写丢 `_version` ⇒ ✅ **真机已验证** |
...
**下一动作**：在 PC-DMIS 里跑 `PC2XL_EXPORT` 一轮同时验 **P1-4 + P1-5 + P1-6 + P1-7 + P1-8**。
P1-7 / P1-8 代码侧已全部就绪（`711753c` + `63b55c0`），且与 P1-5 / P1-6 互不影响。
下一动作是**唯一一次真机验证**（5 个时点见真机清单）。
...
| P1-7 | `part.Save` 在某些 PC-DMIS 2024.1 实例上是**属性且 setter 不存在**（P1-6 没覆盖的第三种形态） | `inject/save_helper.py:_trigger_part_save` | 自动保存恒失败（特定实例） | ✅ | ✅ **已完成 + 单测有牙**（提示补全 `5d11cdd` + SaveAs 降级 `711753c`）；真机待验 |
| P1-8 | BAS 文件用 UTF-8 写，但 PC-DMIS Basic Scripting Engine 按 GBK 解码 → 中文 `MsgBox` 字符串乱码 → Syntax Error | `inject/command_injector.py:deploy_bas_script`（部署出编码） | 功能不可用 | ✅ | ✅ **已完成 + 单测有牙**（`63b55c0`）；真机待验 |
...
**验证（已做 + 待做）**
- 已做（**提示补全** `5d11cdd`）：`_save_failure_hint` 提示包含「setter 不存在」「无降级路径」关键字。+1 条测试 `test_failure_hint_mentions_read_only_form`，已用 `git stash push -- <save_helper.py>` 证明「修复前 1 failed」（`setter 不存在` 不在旧文案里）。
- 已做（**运行时降级** `711753c`）：`_trigger_part_save` 捕获形态 C 的 `part.Save = True` 异常，降级到 `part.SaveAs(part.FullName or part.Path or "")`。+2 条替身测试 `test_save_read_only_falls_back_to_saveas` + `test_save_read_only_no_saveas_reports_failure`，已用 `git stash push -- <save_helper.py>` 证明「修复前 1 failed」（SaveAs 没被调，hit `saveas_call_count == 1` 反证）。降级链全断（SaveAs 也不存在）时 `(False, ...)` + 提示含「无降级路径」—— 不静默吞错。
- 待做：真机端到端（详见文末真机清单）—— 用户机器上点「植入/更新导出命令」应**不再**弹「Property can not be set」，PRG mtime 应更新。

真机：在用户机器上已确认 `SaveAs(part.FullName)` 可调（2026-09-23 真机诊断：`<bound method SaveAs of ...> / callable=True`），无需再决策走哪条。
...
**验证**（已做 + 待做）
- 已做（`63b55c0`）：`command_injector.py:deploy_bas_script` 的 `atomic_write_text(dest, source_text)` 改成 `atomic_write_text(dest, source_text, encoding="gbk")`。+2 条测试 `test_deploy_bas_script_writes_gbk_encoding` + `test_deploy_bas_script_gbk_roundtrip_preserves_chinese`，已用 `git stash push -- <command_injector.py>` 证明「修复前 2 failed」（GBK decode 抛 UnicodeDecodeError）。`test_deploy_bas_script_copies_and_substitutes` 原按 `utf-8` 读回 ⇒ **改了读法跟新契约对齐**（守护的本质契约「占位符被替换、`export_config.txt` 在内容里」没变，只是 encoding 跟着 P1-8 走）。`_write_bas_template` 加了中文 MsgBox 让 P1-8 测试有真东西可断（不是空壳断言）。
- 待做：真机端到端（详见文末真机清单）—— 部署后用 16 进制看 `export_current.bas` 中文部分应是 GBK 字节（不是 UTF-8）；再跑 PC2XL_EXPORT 不再报 Syntax Error。

真机清单该条与 P1-5 + P1-6 + P1-7 合并到「P1-4 + P1-5 + P1-6 + P1-7 + P1-8 端到端」那一条 —— 一次跑完整验。
...
- [ ] **P1-4 + P1-5 + P1-6 + P1-7 + P1-8 端到端一轮**：① 重新部署 BAS 脚本（**P1-5 命中**：自动覆盖 `FLD_` 前缀；**P1-8 命中**：自动按 GBK 写出）② 植入/更新导出命令 —— 应**不再**弹「Property can not be set」**P1-7 命中**（SaveAs 降级路径）；也不再弹「自动保存失败」（P1-6 形态 B/C 都覆盖）；弹「已保存: …PRG」的概率高 ③ 在 PC-DMIS 里把光标移到 `PC2XL_EXPORT` → 文件 → 部分执行 → 从光标执行 ④ `D:\AI\work\cm2xl\data\reports\pcdmis_partial_export.csv` 生成（**P1-5 + P1-4 + P1-8 同时验**）
...
- [ ] **P1-7（独立补充测试）**：用户机器上植入应**不再**弹「Property can not be set」；PRG mtime 应更新（`SaveAs(part.FullName)` 降级成功）；`IsModified` 应转 False
- [ ] **P1-8（独立补充测试）**：部署后 `export_current.bas` 头部应是 GBK 编码（用 16 进制查看：原 UTF-8「无」是 `E6 97 A0`，GBK 是 `CE DE`，有显著区别）；再跑 PC2XL_EXPORT 不再报 Syntax Error
